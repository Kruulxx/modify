                ...
                Section::make('Class Students')->schema([
                    Select::make('students')
                        ...
                        ->options(function () {
                            ...
                            $currentSchoolYear = SchoolYear::where('current', 1)->first();
                
                            // Get the current class's enrolled students
                            $currentClassStudentsJson = DB::table('classes')
                                ->where('school_year_id', $currentSchoolYear->id)
                                ->pluck('students')
                                ->first();
                            
                            // If the class has no enrolled students, return all students
                            if (empty($currentClassStudentsJson)) {
                            ...
                            // Decode the JSON data of students in the current class
                            $currentClassStudents = json_decode($currentClassStudentsJson);
                
                            // Fetch students who are not in the current class
                            $studentsNotInClass = Student::orderBy('lname', 'asc')
                                ->whereNotIn('lrn', $currentClassStudents)
                                ->get()
                                ->map(function ($student) {
                                    // Modify the 'lname' data here
                                    $student['lname'] = $student['lname'] . ', ' . $student['fname'] . ' ' . $student['mname'];
